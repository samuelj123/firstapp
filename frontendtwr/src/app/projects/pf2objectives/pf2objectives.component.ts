...
export class Pf2objectivesComponent implements OnInit {
	...
	contentschedule = ['Daily', 'Weekdays', 'Weekends', 'Weekly', 'Fortnightly', 'Monthly', 'Other' ]
	...
	async ngOnInit() {
		...
		this.myForm = this.fb.group({
			...
			contentschedule: ['', Validators.required],
			programsno: ['', Validators.required],
			productionformat: ['', Validators.required],
			productionkpis: this.fb.array([this.fb.control('')]),
		});

		// Patching Values into the Production form in case of editing older projects
		this.myForm.patchValue({
			programname: this.project.programname,
			contentformat: this.project.contentformat,
			contentdescription: this.project.contentdescription,
			contentschedule: this.project.contentschedule,
			duration: this.project.duration,
			programsno: this.project.programsno,
			productionformat: this.project.productionformat,
		});
